...
const formBuilderSlice = createSlice({
    ...
    reducers: {
        ...
        setBuilderFromSchema: (state, action) => {
            const form = action.payload;
            if (!form) return;

            // Search across ALL common naming conventions for the schema JSON
            const schema = form.schema || form.form_schema || form.schema_body || {};

            // 1. Initial Screen Source
            let sourceScreens = schema.screens || [];

            // 2. Data Recovery Phase 1: Meta-Style Extraction
            // If screens exist but have no fields, check if they have Meta-style layout children
            const screenSourceWithMetaFields = sourceScreens.map(screen => {
                const existingFields = screen.fields || [];
                if (existingFields.length === 0 && screen.layout?.children) {
                    const extracted = screen.layout.children
                        .filter(c => c.type !== 'Footer')
                        .map(c => ({
                            id: c.name || `field_${Math.random().toString(36).substr(2, 9)}`,
                            type: c.type === 'Dropdown' ? 'select' :
                                c.type === 'RadioButtonsGroup' ? 'radio' :
                                    c.type === 'CheckboxGroup' ? 'checkbox' : 'text',
                            label: c.label || 'Field',
                            options: c['data-source'] || c.options || [],
                            validation_rules: { required: c.required }
                        }));
                    return { ...screen, fields: extracted };
                }
                return screen;
            });

            // 3. Data Recovery Phase 2: Flattened Fields Fallback
            let finalScreens = screenSourceWithMetaFields;
            if (finalScreens.length === 0 && form.fields && form.fields.length > 0) {
                console.log(`[FormBuilder] Reconstructing from ${form.fields.length} flattened fields...`);
                finalScreens = [{
                    id: `screen_recovered_${Date.now().toString(36)}`,
                    title: 'Screen 1',
                    fields: form.fields
                }];
            }
            ...
            if (finalScreens.length === 0) {
                finalScreens = [{ id: `screen_${Date.now().toString(36)}`, title: 'Screen 1', fields: [] }];
            }

            // 5. Universal Normalization Phase
            const normalizedScreens = finalScreens.map(screen => ({
                ...screen,
                title: screen.title || 'Screen',
                fields: (screen.fields || []).map(f => {
                    const rawOptions = f.options || f['data-source'] || [];
                    ...
                    // Add default options for selection types if empty
                    if (options.length === 0 && (f.type === 'select' || f.type === 'radio' || f.type === 'checkbox')) {
                        options = [{ value: 'option_1', label: 'Option 1' }];
                    }

                    return {
                        ...f,
                        id: f.id?.toString() || `field_${Math.random().toString(36).substr(2, 9)}`,
                        options
                    };
                })
            }));
            ...
            return {
                ...
                selectedScreenId: normalizedScreens[0]?.id || null,
                selectedFieldId: null,
        ...
        loadDraft: (state, action) => {
            ...
            return {
                ...state,
                ...draft,
                selectedScreenId: draft.screens?.[0]?.id || state.selectedScreenId
            };
        },
