...
export default function ConversationsPage() {
    ...
    function renderMessageContent(text) {
        ...
        if (typeof text === 'string' && (text.startsWith('[INTERACTIVE_FORM]') || text.startsWith('[TEMPLATE]'))) {
            let templateName = 'WhatsApp Template';
            
            if (text.startsWith('[INTERACTIVE_FORM]')) {
                const match = text.match(/Sent template:\s*([^\s|]+)/i);
                if (match && match[1]) {
                    templateName = match[1];
                } else {
                    templateName = text.replace(/\[INTERACTIVE_FORM\]/i, '').split('|')[0].trim();
                }
            } else if (text.startsWith('[TEMPLATE]')) {
                templateName = text.replace(/\[TEMPLATE\]/i, '').replace(/Loop sent\.?/i, '').trim();
            }
