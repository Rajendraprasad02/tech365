...
export default function ConversationsPage() {
    ...
    // Γ£à Polling: Refresh conversations every 10 seconds
    useEffect(() => {
    ...
    // Γ£à WebSocket: Real-Time Updates
    useEffect(() => {
        ...
        ws.onopen = () => {
            console.log("Γ£à [WS] Connected to WebSocket:", wsUrl);
        };
        ...
        ws.onmessage = (event) => {
            try {
                ...
                console.error("Γ¥î [WS] Error processing message:", err);
            }
        };

        ws.onclose = () => console.log("ΓÜá∩╕Å [WS] Disconnected");
        ws.onerror = (error) => console.log("Γ¥î [WS] Error:", error);

        return () => ws.close();
    }, [user, selectedConversation]); // Re-run if selected conversation changes

    // Γ£à Fetch Missing User Details (By ID)
                    ...
                    <div>
                        ...
                        {/* <p className="text-gray-500 text-sm">
                            {isAgentEffective
                                ...
                                : `${filteredConversations.length} conversations ΓÇó ${activeCount} active, ${pendingCount} pending`
                            }
                        </p> */}
                    </div>
                </div>
                                ...
                                <div className="flex flex-col md:flex-row items-center justify-between gap-2 ">
                                    ...
                                    <div className="flex  items-center gap-3 ml-auto">
                                        ...
                                        <div className="flex items-center gap-1.5 px-3 py-1.5 bg-green-50 text-green-700 rounded-full">
                                            {/* <DollarSign size={14} /> */}
                                            <span className="text-sm font-semibold">Γé╣{" "}{selectedConversation.cost}</span>
                                        </div>
